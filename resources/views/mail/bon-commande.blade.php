...
<div class="wrapper">
    <div class="header">
        ...
        <p>Bon de commande — {{ $bonCommande->numero }}</p>
    </div>
    <div class="body">
        <p>Bonjour,</p>
        <p>Veuillez trouver ci-joint votre bon de commande en pièce jointe (PDF).</p>
        ...
        <div class="highlight">
            ...
            <div class="value">{{ number_format($bonCommande->montant_ttc, 2, ',', ' ') }} €</div>
        </div>

        <div class="info-grid">
            <div class="info-item">
                <div class="label">Numéro BDC</div>
                <div class="value">{{ $bonCommande->numero }}</div>
            </div>
            <div class="info-item">
                <div class="label">Date du document</div>
                <div class="value">{{ $bonCommande->date_document->format('d/m/Y') }}</div>
            </div>
            @if($bonCommande->date_debut_travaux)
            <div class="info-item">
                <div class="label">Début des travaux</div>
                <div class="value">{{ $bonCommande->date_debut_travaux->format('d/m/Y') }}</div>
            </div>
            @endif
            @if($bonCommande->chantier)
            <div class="info-item">
                <div class="label">Chantier</div>
                <div class="value">{{ $bonCommande->chantier->nom }}</div>
            </div>
            @endif
        </div>
