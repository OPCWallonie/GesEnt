...
<head>
    ...
    <style>
        ...
        .highlight { background: #f0fdf4; border-left: 4px solid #16a34a; border-radius: 4px; padding: 16px 20px; margin: 24px 0; }
        .highlight .label { font-size: 12px; color: #6b7280; text-transform: uppercase; letter-spacing: .05em; }
        .highlight .value { font-size: 24px; font-weight: 700; color: #15803d; margin-top: 4px; }
        ...
        .echeance { background: #fff7ed; border-left: 4px solid #f97316; border-radius: 4px; padding: 12px 16px; margin: 16px 0; font-size: 14px; }
    </style>
</head>
<body>
<div class="wrapper">
    ...
    <div class="body">
        <p>Bonjour,</p>
        <p>Veuillez trouver ci-joint votre facture en pièce jointe (PDF).</p>
        ...
        <div class="highlight">
            ...
            <div class="value">{{ number_format($facture->montant_net_a_payer, 2, ',', ' ') }} €</div>
        </div>
        ...
        <div class="info-grid">
            <div class="info-item">
                ...
                <div class="value">{{ $facture->numero }}</div>
            </div>
            <div class="info-item">
                <div class="label">Date d'émission</div>
                <div class="value">{{ $facture->date_document->format('d/m/Y') }}</div>
            </div>
            @if($facture->chantier)
            <div class="info-item">
                <div class="label">Chantier</div>
                <div class="value">{{ $facture->chantier->nom }}</div>
            </div>
            @endif
            @if($facture->modePaiement)
            <div class="info-item">
                <div class="label">Mode de paiement</div>
                <div class="value">{{ $facture->modePaiement->nom }}</div>
            </div>
            @endif
        </div>
        ...
        @if($parametres->iban)
            <p style="font-size:13px;color:#6b7280;">
                ...
                <strong>IBAN :</strong> {{ $parametres->iban }}
                @if($parametres->bic) — <strong>BIC :</strong> {{ $parametres->bic }} @endif
