<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between flex-wrap gap-3">
            <div>
                <span class="font-semibold">{{ $absenceCollective->libelle }}</span>
                <span class="text-gray-400 font-normal ml-2 text-base">
                    — {{ ucfirst($absenceCollective->date->translatedFormat('l d/m/Y')) }}
                </span>
                <span class="ml-2 inline-block px-2 py-0.5 rounded-full text-xs
                    {{ $absenceCollective->type_collectif === 'repos_compensatoire' ? 'bg-blue-100 text-blue-700' :
                       ($absenceCollective->type_collectif === 'report_ferie' ? 'bg-purple-100 text-purple-700' : 'bg-green-100 text-green-700') }}">
                    {{ $absenceCollective->label_type }}
                </span>
            </div>
            <div class="flex items-center gap-2">
                @hasanyrole('admin|comptable')
                @if(! $absenceCollective->applique)
                    <form method="POST" action="{{ route('absences-collectives.appliquer', $absenceCollective) }}"
                    ...
                    <form method="POST" action="{{ route('absences-collectives.destroy', $absenceCollective) }}"
                          onsubmit="return confirm('Supprimer cette absence collective ?')">
                        @csrf @method('DELETE')
                ...
                @else
                    <form method="POST" action="{{ route('absences-collectives.annuler', $absenceCollective) }}"
                          onsubmit="return confirm('Annuler et supprimer toutes les absences générées automatiquement ?')">
                        @csrf
                        <button type="submit"
                ...
                @endif
                @endhasanyrole
                <a href="{{ route('absences-collectives.index', ['type' => $absenceCollective->type_collectif]) }}"
    ...
    <div class="mb-4 bg-green-50 border border-green-200 rounded-xl px-5 py-3 flex items-center gap-3 text-sm text-green-800">
        ...
        <span>
            Appliqué le {{ $absenceCollective->applique_le->format('d/m/Y à H:i') }} —
            <strong>{{ $absenceCollective->absences->count() }} absence(s)</strong> générée(s).
        </span>
    </div>
    @endif

    @if($absenceCollective->type_collectif !== 'repos_compensatoire')
    <div class="mb-4 bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 text-sm text-blue-700">
        @if($absenceCollective->type_collectif === 'report_ferie')
            Ce jour est un <strong>report de jour férié</strong> — ne décompte pas du quota RC ni des congés légaux.
        @else
            Ce jour est un <strong>congé d'entreprise</strong> — ne décompte pas du quota RC ni des congés légaux.
        @endif
    </div>
    @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 space-y-4">
            ...
            <div class="space-y-3 text-sm">
                <div class="flex justify-between">
                    ...
                    <span class="font-medium">{{ $absenceCollective->label_type }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-500">Date</span>
                    <span class="font-medium">{{ ucfirst($absenceCollective->date->translatedFormat('l d/m/Y')) }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-500">Durée</span>
                    <span>{{ $absenceCollective->demi_journee ? '½ journée (0,5 j)' : '1 journée complète' }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-500">Périmètre</span>
                    <span>{{ $absenceCollective->libelle_perimetre }}</span>
                </div>
                @if(! empty($absenceCollective->perimetre_valeurs))
                <div class="flex justify-between">
                    <span class="text-gray-500">Valeurs</span>
                    <span class="text-right">{{ implode(', ', $absenceCollective->perimetre_valeurs) }}</span>
                </div>
                @endif
                @if($absenceCollective->notes)
                <div class="pt-2 border-t border-gray-100">
                    <p class="text-gray-500 text-xs mb-1">Notes</p>
                    <p class="text-gray-700">{{ $absenceCollective->notes }}</p>
                </div>
                @endif
            </div>
        </div>

        {{-- Avant application : conflits + personnel --}}
        @if(! $absenceCollective->applique)
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="bg-amber-50 border border-amber-200 rounded-xl p-5">
                ...
                <p class="text-xs text-amber-700 mb-2">Ces membres ont déjà une absence ce jour-là et seront ignorés :</p>
        ...
        {{-- Après application : absences générées --}}
        @else
        <div class="lg:col-span-2">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-100">
                    <h3 class="font-semibold text-gray-700 text-sm">
                        Absences générées ({{ $absenceCollective->absences->count() }})
                    </h3>
                </div>
                @if($absenceCollective->absences->isEmpty())
                ...
                <table class="min-w-full text-sm divide-y divide-gray-100">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @foreach($absenceCollective->absences->sortBy(fn($a) => $a->ouvrier->nom) as $absence)
