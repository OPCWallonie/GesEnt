<x-app-layout>
    ...
    <x-slot name="actions">
        <a href="{{ route('bons-commande.create') }}"
    ...
    {{-- Filtres --}}
    <form method="GET" action="{{ route('bons-commande.index') }}" class="mb-6 flex flex-wrap gap-3 items-end">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Devis lié</th>
                ...
                @forelse($bonsCommande as $bdc)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            @if($bdc->devis)
                                <a href="{{ route('devis.show', $bdc->devis) }}"
                                   class="text-blue-500 hover:text-blue-700">{{ $bdc->devis->numero }}</a>
                            @else
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $bdc->date_document->format('d/m/Y') }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <x-badge :statut="$bdc->statut"/>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 text-right font-medium">
                            {{ number_format($bdc->montant_ttc ?? 0, 2, ',', ' ') }} €
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end gap-3">
                                <a href="{{ route('bons-commande.show', $bdc) }}" class="text-gray-500 hover:text-gray-700" title="Voir">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                    </svg>
                                </a>
                                @if((string) $bdc->statut !== 'archive')
                                    <a href="{{ route('bons-commande.edit', $bdc) }}" class="text-blue-500 hover:text-blue-700" title="Modifier">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @if($bonsCommande->hasPages())
            <div class="px-6 py-4 border-t border-gray-200">
                {{ $bonsCommande->withQueryString()->links() }}
            </div>
        @endif
    </div>
</x-app-layout>
