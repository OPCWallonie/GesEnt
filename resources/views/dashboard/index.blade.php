<x-app-layout>
    ...
    @if($certificationsARenouveler->count() > 0)
        <div class="bg-yellow-50 border border-yellow-300 rounded-xl p-4 mb-4">
            <div class="flex items-center gap-2 mb-2">
                ...
                <span class="font-semibold text-yellow-800">{{ $certificationsARenouveler->count() }} certification(s) à renouveler (90 jours)</span>
            </div>
            <div class="space-y-1">
                @foreach($certificationsARenouveler as $cert)
                    <div class="flex items-center justify-between text-sm text-yellow-800">
                        <a href="{{ route('ouvriers.show', $cert->ouvrier) }}" class="hover:underline font-medium">{{ $cert->ouvrier->nom_complet }}</a>
                        <span class="text-gray-600">{{ $cert->libelle_type }}</span>
                        @if($cert->est_expiree)
                            <span class="text-red-600 font-semibold">Expirée le {{ $cert->date_expiration->format('d/m/Y') }}</span>
                        @else
                            <span>Expire le {{ $cert->date_expiration->format('d/m/Y') }}</span>
                        @endif
                    </div>
                @endforeach
            </div>
